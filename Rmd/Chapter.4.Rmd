...
knitr::opts_chunk$set(
  dev        = "png",
  fig.path   = paste0(here::here("figure", "Chapter4"), "/plot-"),
  fig.width  = 7,
  fig.height = 5
)
```
...
```{r}
ruta_guardado <- normalizePath(here("figure/Chapter4"), winslash = "/")
# Crear carpeta si no existe
if (!dir.exists(ruta_guardado)) {
  dir.create(ruta_guardado, recursive = TRUE)
}
# Inicializar contador de gráficos
plot_counter <- 1

# Función para guardar gráficos automáticamente
guardar_plot <- function(plot) {
  globalenv()$plot_counter <- globalenv()$plot_counter + 1  # Incrementar contador
  nombre_archivo <- file.path(ruta_guardado, paste0("grafico_", plot_counter, ".png"))
  ggsave(nombre_archivo, plot = plot, width = 8, height = 6, dpi = 300)
}
```
...
```{r,results = "hide"}
sp1<-metaMDS(sp,autotransform = FALSE, distance = "bray", k = 2, trymax = 999)
```
...
A priori, the Auchenorrhyncha community is different in olive plots. The models will be used to further deepen the analysis.
